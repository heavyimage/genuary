class Infinitan {
  ...
  Infinitan(int c_idx) {
    ...
    flitting = false;
    //flitspeed = random(0.0005, 0.01);
    flitspeed = max(0.00001, min(abs(randomGaussian()), 0.01));
  ...
  void move() {
    ...
    if (!flitting && flitcount < 100 && frameCount % (offset+1) == 0 && random(0, 1) > 0.1) {
      flitting = true;
      target.set(
        current.x + abs((width/5)*randomGaussian()),
        current.y + abs((height/5)*randomGaussian())
      );
      //randompos(target);
      progress = 0;
      start = current.copy();
